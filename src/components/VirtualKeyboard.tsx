...
export default function VirtualKeyboard({
  ...
  const getKeyColorClass = (key: string): string => {
    ...
    switch (status) {
      ...
      case 'ABSENT':
        return 'bg-ash-absent text-linen-bg/85 border-ash-absent hover:opacity-90 opacity-60';
      default:
  ...
  return (
    <div className="w-full max-w-lg mx-auto py-2 px-1 select-none">
      <div 
      ...
      >
        {rows.map((row, rowIndex) => (
          <div 
            key={rowIndex} 
            className="flex justify-center gap-1 sm:gap-1.5 touch-manipulation"
          >
            {row.map((key) => {
              const isSpecialKey = key === 'ENTER' || key === 'DELETE';
              const colorClass = getKeyColorClass(key);
              
              // Special size classes for Action keys
              const sizeClass = isSpecialKey 
                ? 'px-2 sm:px-4 text-[10px] sm:text-xs font-bold uppercase tracking-wider flex-1.5 min-w-[45px] sm:min-w-[65px]'
                : 'w-8 h-10 sm:w-10 sm:h-12 text-sm sm:text-base font-semibold uppercase flex-1';
              ...
              return (
                <button
                  ...
                  className={`
                    h-11 sm:h-12 flex items-center justify-center rounded-lg border-b-2
